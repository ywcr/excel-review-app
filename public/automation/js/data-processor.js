            ...
            reader.onload = (e) => {
                try {
                    ...
                    for (let i = 0; i < data.SheetNames.length; i++) {
                        ...
                        const sheet = XLSX.utils.sheet_to_json(data.Sheets[sheetName], { header: 1 });
                        this.sheets[sheetName] = sheet;
                    }

                    Logger.logSuccess(`✅ 文件加载成功: ${fileName}`);
                    Logger.logInfo(`📊 工作表数量: ${data.SheetNames.length}`);
...
class SheetMatcher {
    ...
    findMatchingSheet(sheets, preferences = {}) {
        ...
        if (preferredSheet && sheets[preferredSheet]) {
            ...
            return { found: true, sheetName: preferredSheet, reason: 'user_preference' };
        }
        ...
        if (sheets[this.config.sheetName]) {
            ...
            return { found: true, sheetName: this.config.sheetName, reason: 'exact_match' };
        }
        ...
        return {
            found: false,
            reason: `未找到包含产品关键词 [${this.config.keywords.filter(k => CONFIG.productKeywords.includes(k)).join(', ')}] 的工作表`,
    ...
    findFuzzyMatch(sheetNames, keywords) {
        ...
        const currentProductKeyword = keywords.find(k => CONFIG.productKeywords.includes(k));
        ...
        sheetNames.forEach(sheetName => {
            ...
            Logger.logInfo(`   "${sheetName}": 得分${score}, 产品关键词${hasProductKeyword ? '✅' : '❌'}, 匹配词[${matchedKeywords.join(', ')}]`);
