...
class QuestionnaireTypeSelector {
  ...
  selectType(type) {
    ...
    DOMUtils.addClass(
      DOMUtils.querySelector(`[data-type="${type}"]`),
      "active"
    );
  ...
  getSelectedConfig() {
    return this.selectedType
      ? CONFIG.questionnaireTypes[this.selectedType]
      : null;
  }
}
...
class FileUploader {
  ...
  initialize() {
    ...
    // 防止重复绑定事件
    if (this.initialized) {
      console.log("FileUploader already initialized, skipping...");
      return;
    }
...
class DataPreviewer {
  ...
  displayPreview(data) {
    ...
    data.slice(0, CONFIG.defaults.maxPreviewData).forEach((item, index) => {
      previewText += `${index + 1}. 姓名: ${item.name}, 性别: ${
        item.sex
      }, 时间: ${item.time}, 指派人: ${item.assignee}\n`;
      if (item.hospital) {
...
class AssigneeManager {
  ...
  selectAssignee(assignee) {
    ...
    DOMUtils.addClass(
      DOMUtils.querySelector(`[data-assignee="${assignee}"]`),
      "selected"
    );
...
class DateManager {
  ...
  displayAssigneeDates(assignee, data) {
    ...
    if (!data || data.length === 0) {
      this.statusContainer.innerHTML =
        '<p style="text-align: center; color: #6c757d; padding: 20px;">该指派人暂无数据</p>';
      return;
    }
  ...
  selectDate(date, data) {
    ...
    DOMUtils.addClass(
      DOMUtils.querySelector(`[data-date="${date}"]`),
      "current"
    );
