...
describe('FrontendValidator Complex Rules', () => {
  let validator: FrontendValidator;

  beforeEach(() => {
    const template = TASK_TEMPLATES['药店拜访'];
    validator = new FrontendValidator(template);
  });
  ...
  describe('Unique Rule Validation', () => {
    it('should detect duplicate values in unique fields', async () => {
      ...
      const result = await validator.validateWorkbook(workbook, 'Sheet1');

      // 应该检测到重复的零售渠道
      const uniqueErrors = result.errors.filter(error => error.type === 'unique');
  ...
  describe('Frequency Rule Validation', () => {
    it('should detect frequency violations', async () => {
      ...
      // 应该检测到频次超限
      const frequencyErrors = result.errors.filter(error => error.type === 'frequency');
  ...
  describe('Date Interval Rule Validation', () => {
    it('should detect date interval violations', async () => {
      ...
      // 应该检测到日期间隔违规
      const intervalErrors = result.errors.filter(error => error.type === 'dateInterval');
  ...
  describe('Time Range Rule Validation', () => {
    it('should detect time range violations', async () => {
      ...
      // 应该检测到时间范围违规
      const timeRangeErrors = result.errors.filter(error => error.type === 'timeRange');
  ...
  describe('Duration Rule Validation', () => {
    it('should detect duration violations', async () => {
      ...
      // 应该检测到持续时间违规
      const durationErrors = result.errors.filter(error => error.type === 'duration');
  ...
  describe('Required Field Validation', () => {
    it('should detect missing required fields', async () => {
      ...
      // 应该检测到必填字段缺失
      const requiredErrors = result.errors.filter(error => error.type === 'required');
